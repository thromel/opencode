...
- `context_ledger` - Include a provenance-preserving context packet in compaction prompts. `policy` selects the ContextLedger strategy and `budget` controls the packet token budget. `mode` defaults to `augment`, which adds the packet beside the raw old history; experimental `replace` mode uses the packet instead of sending that raw old history to the summarizer. Use `precision-frontier` for replacement-mode experiments that should keep exact facts while demoting distractor/noise lines.
